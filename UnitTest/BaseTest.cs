...
namespace UnitTest
{
  ...
  public class BaseTest : Xunit.IUseFixture<Fixture>, IDisposable
  {
    ...
    protected void InitHttpContextMock(string queryString)
    {
      ...
      AddServerVariable(HttpContext.Current.Request, "SERVER_PORT", null);
      AddServerVariable(HttpContext.Current.Request, "HTTP_X_REWRITE_URL", null);

      Sdx.Context.Current.Timer.Start();
      Sdx.Context.Current.Debug.Out = Console.Out;
    }

    public static void AddServerVariable(HttpRequest request, string key, string value)
    {
